...
public class AvatarMeshCombiner : IDisposable
{
    ...
    /// The combine option
    /// </summary>
    private CombineOption combineOption;
    /// <summary>
    ...
    public AvatarMeshCombiner(CombineOption combineOption)
    {
        this.combineOption = combineOption;
    }
    /// <summary>
    ...
    /// <param name="combineOption">The combine option.</param>
    public static void CombineSkinnedMesh(SkinnedMeshRenderer[] targetMeshes,SkinnedMeshRenderer combinedMesh, CombineOption combineOption)
    {
        if (targetMeshes==null|| targetMeshes.Length==0)
            return;
        using (AvatarMeshCombiner combiner = new AvatarMeshCombiner(combineOption))
        {
        ...
        if (combineOption.combineFlags.HasFlag(CombineOption.CombineFlags.RemoveTargetMeshes))
    ...
    private void ApplySkinnedMeshs(SkinnedMeshRenderer[] targetMeshes)
    {
        ...
        this.meshCombineDescriptorsSet = InitMeshCombineDescriptor(combineOption, targetMeshes);
        ...
        foreach (var meshCombineDescriptor in meshCombineDescriptors)
        {
            ...
            if (!meshCombineDic.ContainsKey(parentMesh))
            {
                ...
                for (int i = 0; i < bones.Length; i++)
                {
                    ...
                    Matrix4x4 bindpose = bindposeList[i] * parentMesh.transform.worldToLocalMatrix;
                ...
                meshCombineDic[parentMesh] = (boneWeightList, parentMesh.localToWorldMatrix, mesh);
            }
            ...
            var copyMaterial = combineOption.combineFlags.HasFlag(CombineOption.CombineFlags.IncludeTextureAtlasing) ? false : combineOption.combineFlags.HasFlag(CombineOption.CombineFlags.RemoveTargetMeshes) ? true : false;
            ...
            meshCombineDescriptor.material = copyMaterial ? AvatarMaterialCombiner.CopyNewMaterial(material, combineOption) : material;
    ...
    private Mesh Combine(SkinnedMeshRenderer destination)
    {
        ...
        if (combineOption.combineFlags.HasFlag(CombineOption.CombineFlags.IncludeTextureAtlasing))
        {
            if (!combineOption.TextureAtlasOptimization || (combineOption.TextureAtlasOptimization && meshCombineDescriptorsSet.Count==1))
            {
        ...
        if (!combineOption.combineFlags.HasFlag(CombineOption.CombineFlags.RemoveBlendshapes))
        {
            int offset = 0;
            foreach (var combine in combineInstanceList)
            {
    ...
    private Material GetCombinedMaterial(Mesh combinedNewMesh, List<MeshCombineDescriptor> combineDescriptorGroup)
    {
        ...
        var (materialTextureInfoDic, combinedMaterial) = AvatarMaterialCombiner.CombineMaterial(materialGroup, combineOption);
        ...
        foreach (var combineDescriptor in combineDescriptorGroup)
        {
            ...
            textureInfoBase.offsetRatio.y = 1 - (textureInfoBase.offsetRatio.y + textureInfoBase.sizeRatio.y);
    ...
    private static List<List<MeshCombineDescriptor>> InitMeshCombineDescriptor(CombineOption combineOption, SkinnedMeshRenderer[] targetMeshes)
    {
        ...
        if (combineOption.combineFlags.HasFlag(CombineOption.CombineFlags.IncludeTextureAtlasing) && combineOption.TextureAtlasOptimization)
    ...
    private static bool MaterialCombineCheck(SkinnedMeshRenderer[] targetMeshes)
    {
        foreach (var sm in targetMeshes)
            foreach (var materia in sm.materials)
                if (!AvatarMaterialCombiner.MaterialCombineVerification(materia))
                    return false;
        return true;
    }
    /// <summary>
    ...
    /// <returns></returns>
    public static Renderer InstantiateRendererParms(Renderer renderer)
    {
        ...
        renderer.material = AvatarMaterialCombiner.CopyNewMaterial(renderer.material);
        return renderer;
    }
