Shader "Custom/AssetPBR" {
	Properties{
		...
		u_alpha_cutoff("AlphaCutoff", Float) = 0
	...
	v2f vert(appdata_tan input)
	{
		...
		float3 normal = normalize(mul(normal_matrix, float4(transformed_normal, 0.0))).xyz;   // N
		output.tbn_normal = normal;
		// on by default
		if (1 == u_primitive_usage.IDX_NORMAL_MAP_VERTEX) {
			float3 tangent = normalize(mul(normal_matrix, float4(input.tangent.xyz, 0.0))).xyz;   // T
		...
		output.v_normal_refl = normalize(mul(mul(u_ReflMatrix, normal_matrix), float4(transformed_normal, 0.0))).xyz;
	...
	float3 getDirectLight(float3 V, float3 N, float3 v_model_pos, float3 specularEnvironmentR0, float3 specularEnvironmentR90, float NdotV, float roughnessBiq, float3 diffuseColor, int idx)
	{
		if (u_light_intensity[idx] != 0.0) {
			...
			if (NdotL > 0.0) {
				...
				float3 specular_BRDF = F * G * D / (4.0 * NdotL * NdotV);           // Specular BRDF : Cook-Torrance
	...
	float4 frag(v2f input) : SV_Target
	{
        ...
        // Transparency removed since Phone.apk doesn't support transparency
        baseColor.a = 1;
...
//			}
		...
		float3 specularColor = lerp(float3(M_F03), baseColor.rgb, metallic);
		...
		// Indirect Light : IBL
		float3 reflection = -normalize(reflect(V, input.tbn_normal));
...
//		color = pow(color, float3(1.0 / M_GAMMA, 1.0 / M_GAMMA, 1.0 / M_GAMMA));
		float4 colorResult = fixed4(LINEARtoSRGB(color), alpha);

//        return fixed4(SRGBtoLINEAR(texCUBElod(specularIBLSampler, float4(reflection, /*roughness * */8.0)).rgb * u_light_intensity[IDX_LIGHT_IBL_SPECULAR]), 1);
		return colorResult;
	}
		ENDCG
	} // pass
